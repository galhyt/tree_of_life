(ns tree-of-clj.core
  (:require [tree-of-clj.helper :refer [get-value-by-path iterate-on-tree iterations-on-tree]]))
...
(defn query [tree-state rule iterations-number path]
  ...
  (let [new-tree (iterations-on-tree tree-state rule iterations-number)]
    (get-value-by-path new-tree path)))
;
;(defn game-of-life
